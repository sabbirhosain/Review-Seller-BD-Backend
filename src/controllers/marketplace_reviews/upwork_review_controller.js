...
export const create = async (req, res) => {
    try {
        ...
        const exist_items = await Upwork_Reviews_Model.exists({ $or: [{ item_name: { $regex: new RegExp(`^${item_name.trim()}$`, 'i') } }] })
        ...
        const result = await new Upwork_Reviews_Model({
            item_name: item_name,
...
export const show = async (req, res) => {
    try {
        ...
        const result = await Upwork_Reviews_Model.find(dataFilter)
            .sort({ createdAt: -1 })
            .limit(limit)
            .skip((page - 1) * limit)

        const count = await Upwork_Reviews_Model.find(dataFilter).countDocuments();
...
export const single = async (req, res) => {
    try {
        ...
        const result = await Upwork_Reviews_Model.findById(id);
        if (!result) {
            return res.json({ success: false, message: "No Data Found" });
        } else {
            return res.json({
...
export const update = async (req, res) => {
    try {
        ...
        const find_items = await Upwork_Reviews_Model.findById(id);
        ...
        const exist_items = await Upwork_Reviews_Model.findOne({ $or: [{ item_name: { $regex: new RegExp(`^${item_name.trim()}$`, 'i') } }], _id: { $ne: new mongoose.Types.ObjectId(id) } });
        ...
        const result = await Upwork_Reviews_Model.findByIdAndUpdate(id, {
            item_name: item_name,
            categories_id: categories_id,
            categories_name: find_categories.categories_name,
            features: features,
...
export const destroy = async (req, res) => {
    try {
        ...
        const result = await Upwork_Reviews_Model.findByIdAndDelete(id);
