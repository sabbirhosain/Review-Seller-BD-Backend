...
export const show = async (req, res) => {
    try {
        ...
        const result = await Fiverr_Reviews_Model.find(dataFilter)
            .sort({ createdAt: -1 })
            .limit(limit)
            .skip((page - 1) * limit)

        const count = await Fiverr_Reviews_Model.find(dataFilter).countDocuments();
...
export const single = async (req, res) => {
    try {
        ...
        const result = await Fiverr_Reviews_Model.findById(id);
        if (!result) {
            return res.json({ success: false, message: "No Data Found" });
        } else {
            return res.json({
