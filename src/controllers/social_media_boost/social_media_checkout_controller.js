...
import Social_Media_Checkout_Model from "../../models/social_media_boost/social_media_checkout_model.js";
import Facebook_Boost_Model from "../../models/social_media_boost/facebook_boost_model.js";
import Youtube_Boost_Model from "../../models/social_media_boost/youtube_boost_model.js";
...
export const create = async (req, res) => {
    try {
        ...
        const find_items = await Facebook_Boost_Model.findById(item_id) || await Youtube_Boost_Model.findById(item_id) || await Google_Ads_Model.findById(item_id);
        ...
        // Save to DB
        const result = await new Social_Media_Checkout_Model({
...
export const show = async (req, res) => {
    try {
        ...
        const result = await Social_Media_Checkout_Model.find(dataFilter)
            .sort({ createdAt: -1 })
            .limit(limit)
            .skip((page - 1) * limit)

        const count = await Social_Media_Checkout_Model.find(dataFilter).countDocuments();
...
export const single = async (req, res) => {
    try {
        ...
        // Check not found
        const result = await Social_Media_Checkout_Model.findById(id);
        if (!result) {
...
export const update = async (req, res) => {
    try {
        ...
        const find_items = await Social_Media_Checkout_Model.findById(id);
        ...
        // update
        const result = await Social_Media_Checkout_Model.findByIdAndUpdate(id, {
...
export const destroy = async (req, res) => {
    try {
        ...
        const result = await Social_Media_Checkout_Model.findByIdAndDelete(id);
