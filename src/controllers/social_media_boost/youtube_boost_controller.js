...
export const create = async (req, res) => {
    try {
        ...
        const exist_items = await Youtube_Boost_Model.exists({ $or: [{ item_name: { $regex: new RegExp(`^${item_name.trim()}$`, 'i') } }] })
        if (exist_items) { return res.json({ success: false, message: "already exists. try another" }) }

        const result = await new Youtube_Boost_Model({
            item_name: item_name,
...
export const show = async (req, res) => {
    try {
        ...
        const result = await Youtube_Boost_Model.find(dataFilter)
            .sort({ createdAt: -1 })
            .limit(limit)
            .skip((page - 1) * limit)

        const count = await Youtube_Boost_Model.find(dataFilter).countDocuments();
...
export const update = async (req, res) => {
    try {
        ...
        const find_items = await Youtube_Boost_Model.findById(id);
        ...
        const exist_items = await Youtube_Boost_Model.exists({ $or: [{ item_name: { $regex: new RegExp(`^${item_name.trim()}$`, 'i') }, _id: { $ne: id } }] })
        if (exist_items) { return res.json({ success: false, message: "already exists. try another" }) }

        // Proceed to create
        const result = await Youtube_Boost_Model.findByIdAndUpdate(id, {
            item_name: item_name,
            categories_id: categories_id,
            categories_name: find_categories.categories_name,
            features: features,
...
export const destroy = async (req, res) => {
    try {
        ...
        // Proceed to delete
        const result = await Youtube_Boost_Model.findByIdAndDelete(id);
        if (!result) {
